...
int main (int argc, char *argv [])
{
    ...
    rc = zsock_bind (cloudfe, "ipc://%s-cloud.ipc", self);
    assert( rc == 0 );
    ...
    for (argn = 2; argn < argc; argn++) {
        ...
        rc = zsock_connect (cloudbe, "ipc://%s-cloud.ipc", peer);
        assert( rc == 0 );
    }
    //  Prepare local frontend and backend
    zsock_t *localfe = zsock_new (ZMQ_ROUTER);
    rc = zsock_bind (localfe, "ipc://%s-localfe.ipc", self);
    assert( rc == 0 );
    zsock_t *localbe = zsock_new (ZMQ_ROUTER);
    rc = zsock_bind (localbe, "ipc://%s-localbe.ipc", self);
    assert( rc == 0 );
    ...
    void *localfe_h = zsock_resolve(localfe);
    void *localbe_h = zsock_resolve(localbe);
    void *cloudfe_h = zsock_resolve(cloudfe);
    void *cloudbe_h = zsock_resolve(cloudbe);

    while (true) {
        //  First, route any waiting replies from workers
        zmq_pollitem_t backends [] = {
            { localbe_h, 0, ZMQ_POLLIN, 0 },
            { cloudbe_h, 0, ZMQ_POLLIN, 0 }
        };
        //  If we have no workers, wait indefinitely
        rc = zmq_poll (backends, 2, capacity? 1000 * ZMQ_POLL_MSEC: -1);
        if (rc == -1)
            break;              //  Interrupted
        ...
        while (capacity) {
            zmq_pollitem_t frontends [] = {
                { localfe_h, 0, ZMQ_POLLIN, 0 },
                { cloudfe_h, 0, ZMQ_POLLIN, 0 }
            };
